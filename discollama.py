...
class Discollama:
    ...
    async def on_message(self, message):
        ...
        if content.startswith('!setmodel '):
            new_model = content.split(' ', 1)[1]
            await self.set_model(new_model, message.channel)
            return
    ...
    async def set_model(self, model_name, channel):
        self.model = model_name
        self.redis.set('discollama:model', model_name)
        await channel.send(f'Model has been set to {model_name}')

    async def get_model(self):
        model = self.redis.get('discollama:model')
        if model:
            self.model = model
...
def main():
    ...
    parser.add_argument('--buffer-size', default=os.getenv('BUFFER_SIZE', 32), type=int)
